...
static const int PRECISION_FRACTION_LIMIT[10] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};
...
struct DateTime 
{
  ...
  static const int64_t  NANOS_PER_DAY = 86400000000000ULL;
  static const int64_t  NANOS_PER_HOUR = 3600000000000ULL;
  static const int64_t  NANOS_PER_MIN = 60000000000ULL;
  static const int64_t  NANOS_PER_SEC = 1000000000ULL;
  ...
  : m_date(src.m_date), m_time(src.m_time) {}
            ...
            int hour, int minute, int second, int millis ) 
  {
    ...
    m_time = makeHMS( hour, minute, second, millis * 1000000 );
  }
  ...
  static inline int convertToNanos(int fraction, int precision)
  {
    int mul = PRECISION_FACTOR[ (unsigned)precision < 10 ? precision : 9 ];
    return fraction * mul;
  }
  ...
  {
    int denom = PRECISION_FACTOR[ (unsigned)precision < 10 ? precision : 9 ];
    return getNanosecond() / denom;
  }
  ...
  static DateTime fromTm( const tm& tm, int millis = 0 )
  {
    return DateTime ( julianDate(tm.tm_year + 1900, tm.tm_mon + 1,
                                 tm.tm_mday),
                     makeHMS(tm.tm_hour, tm.tm_min, tm.tm_sec, millis * 1000000) );
  }
...
public:
  ...
  explicit LocalTimeOnly( time_t time, int fraction, int precision )
  : DateTime( fromLocalTimeT (time, fraction, precision) )
  {
    clearDate();
  }
